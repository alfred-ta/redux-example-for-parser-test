...
function init() {
  ...
  hooksReduxSDK.register.header({
    ...
    // component: "main.html",
    onClick: () => {
      hooksReduxSDK.actions.home.HOME_PAGE_LOADED([{ tags: ['alfred'] }, { articles: [], articlesCount: 0 }])
    }
  });
  ...
  hooksReduxSDK.register.header({
    ...
    onClick: async () => {
      const storeVal = await hooksReduxSDK.state.get('redux');
      alert('store val = ' + storeVal);
    }
  });

  hooksReduxSDK.state.set('redux', 'alfred')
}

window.hooksReduxSDK.onReady(init);